...
編隊敵機、POW 道具、**翻滾迴避（roll）**、**機砲 / 穿透雷射雙武器**、多關卡與巨型頭目戰。
...
| `X` / `Shift` / `K` | **翻滾迴避（ROLL）**：0.9 秒無敵，每次消耗 1 次 |
| `C` / `L` | 炸彈：清除全畫面敵彈並重創敵機 |
| `P` / `Esc` | 暫停 |
| `M` | 靜音切換 |
| `F` / `F11` | 全螢幕切換（畫面等比放大並置中） |
| `Q` | **離開遊戲**（遊戲中會先詢問確認） |
| `Enter` | 標題畫面開始 / 結束後回標題 |
...
- **炸彈或翻滾用完時再按 `C` / `X`，畫面中央會出現 `NO BOMBS LEFT` / `NO ROLLS LEFT` 提示（約 1.5 秒）。**
